...
def calculate_spillover(size, model):
    path = f"./benchmarks/data_{size}_{model}"
    print(path)
    checkpoint_mgr = JSONLCheckpointManager(path)

    projected_clusters = checkpoint_mgr.load_checkpoint(
        "dimensionality", ProjectedCluster
    )
    conversations = checkpoint_mgr.load_checkpoint("conversations", Conversation)
    print(
        f"Loaded {len(projected_clusters)} projected clusters and {len(conversations)} conversations"
    )
...
size = [500, 1000, 6000]
model = ["gpt_4o_mini", "gpt_4.1_mini"]

for s in size:
    for m in model:
        print(f"For size {s} and model {m}:")
        calculate_spillover(s, m)
