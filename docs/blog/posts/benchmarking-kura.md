...
1. **Fast, predictable processing**: 6,000 conversations analyzed in under under 7 minutes and just around $2 in token costs
2. **Negligible storage overhead**: 440x compression ratios mean even 100,000 conversations require only 20MB of storage
...
We then used GPT-4o-mini and GPT-4.1-mini in two separate runs, seeing how the two different models might differ in terms of their performance and cost.
...
| Model            | Dataset Size | Summarisation | Other Steps | **Total** |
| ---------------- | ------------ | ------------- | ----------- | --------- |
| **GPT-4o-mini**  | 500          | 0.14          | 0.03        | **0.17**  |
|                  | 1000         | 0.27          | 0.05        | **0.32**  |
|                  | 6000         | 1.69          | 0.36        | **2.05**  |
| **GPT-4.1-mini** | 500          | 0.50          | 0.00        | **0.50**  |
|                  | 1000         | 0.75          | 0.18        | **0.93**  |
|                  | 6000         | 4.73          | 1.21        | **5.94**  |
...
| Model            | Dataset Size | Summarisation | Other Steps | **Total**  |
| ---------------- | ------------ | ------------- | ----------- | ---------- |
| **GPT-4o-mini**  | 500          | 26.90         | 11.41       | **38.50**  |
|                  | 1000         | 52.50         | 12.57       | **65.10**  |
|                  | 6000         | 320.00        | 94.89       | **414.90** |
| **GPT-4.1-mini** | 500          | 33.20         | 4.99        | **38.20**  |
|                  | 1000         | 52.50         | 14.62       | **67.10**  |
|                  | 6000         | 346.00        | 120.02      | **466.00** |
...
Storage was a concern because Kura caches intermediate results at multiple stages. This allows us to resume in the event of failure easily but also might result in a critical bottleneck when processing large conversation datasets.
...
This has relatively minimal requirements, with 100,000 conversations requiring less than 15MB of storage.

More importantly, as we cluster and summarize increasingly larger datasets, this compression becomes increasingly important. Parquet enables us to store the same data in just a few megabytes while individual summary caching adds virtually no storage burden while providing massive performance improvements.

### Clustering Accuracy

Since the MT-Bench dataset contains identical prompts answered by different AI models, all responses to the same question should be in the same clusters irregardless of the model. This is because the underlying user intent and topic remains consistent.
